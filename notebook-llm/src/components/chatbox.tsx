...
import MDXMessage from "./mdx-message";
import { Message, useChat } from "@/context/message-context";
...
const ChatBox = () => {
  ...
  return (
    <Card className="w-full flex flex-col p-0 gap-0 max-w-4xl min-h-[calc(100vh-100px)] max-h-[calc(100vh-100px)]  h-full border-2  bg-white overflow-hidden ">
      <CardHeader className="whitespace-nowrap px-6 py-4   flex justify-between items-center flex-shrink-0 text-neutral-950 shadow-sm">
      ...
      <CardContent className="flex-1 overflow-y-auto overflow-x-hidden pt-8 px-6  via-white to-orange-50/50 space-y-6 h-0 scrollbar-hide">
        {messages.map((msg, index) => (
          ...
          >
            <div
              className={cn(
                "max-w-2xl flex gap-3 justify-center items-start",
                msg.role === "user" ? "flex-row-reverse" : "flex-row pl-6"
              )}
            >
